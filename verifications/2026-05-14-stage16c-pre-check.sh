#!/usr/bin/env bash
# 16(c) 묶음 3 진입 사전 검증 — DART / naver / KIS 회복 측정
#
# 본 검증 통과 시에만 scan_execute 진입.
# 사전 호출 0 정책 — 본 스크립트 호출 직전까지 다른 호출 0.
#
# 환경 변수: DART_API_KEY (필수)
# 종료 코드: 0 = 통과 / 1 = 영역 어긋남 (보고 후 대기)
#
# Ref: ADR-0015, ADR-0016, ADR-0017, 16(b) 학습 7+8
...
# === 종합 결과 ===
if [ "$PASS" = "true" ]; then
  echo "=== ✓ 사전 검증 통과 — scan_execute 진입 가능 ==="
  echo "다음 단계: sagyeongin_scan_execute 실 호출 (즉시 진입 권고)"
  exit 0
else
  echo "=== ✗ 사전 검증 영역 어긋남 — scan_execute 진입 X ==="
  echo "Onev 보고 후 대기"
  exit 1
fi
