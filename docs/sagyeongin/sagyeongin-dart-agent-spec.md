...
상위 N개(기본 30~50)를 `composite_score = (gap_to_fair ?? 0) × 1.5 + opportunity_score − concern_score` 정렬로 반환.
...
```typescript
{
  ...
  allow_over_daily_limit?: boolean = false,  // pre-check 사전 차단 무력화 (ADR-0019 후속)
}
```
...
```typescript
{
  ...
  pipeline_stats: {
    ...
    override_applied: boolean,   // allow_over_daily_limit 적용 여부
  },
  candidates: Array<{
    rank: number,
    corp_code: string,
  ...
  interpretation_notes: string[],   // override 적용 시 사실 진술 (ADR-0019 후속, ADR-0025)
  next_actions_suggested: string[],
}
```

**composite_score 산식 (ADR-0029)**:
```
composite_score = (srim.gap_to_fair ?? 0) × SRIM_GAP_WEIGHT + (capex.opportunity_score ?? 0) − (cashflow.concern_score ?? 0)
```
`SRIM_GAP_WEIGHT = 1.5` (잠정 — field-test 갭 분포 후 조정. 근거 ADR-0029).
7부 D(저평가 갭)가 정렬 주도, 7부 C(capex)는 tie-breaker 보조.
범위: 갭 0~50 × 1.5 + capex 0~110 − concern 0~100 (실질 범위 확장, 사전 검증 영역).
...
**override (`allow_over_daily_limit`)**: true 시 `usage_pct > 100` 사전 차단(DailyLimitPreCheckError)을 건너뛰고 진입. 사전 차단만 무력화 — 실행 중 한도 80% checkpoint(ADR-0012)는 존속. 신규 scan 첫 진입에만 적용(resume 무관). 적용 시 `override_applied: true` + `interpretation_notes`에 사실 진술. input > preset > false 우선순위. 향후 cache-miss 비율 높을 시 추정 부정확성 진술 추가 가능. ADR-0019 후속 결정 참조.
