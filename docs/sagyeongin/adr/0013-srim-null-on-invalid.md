...
## 적용 사례 — `financial-extractor` throw chain (2026-05-08)

14단계 (b) 묶음 1·2에서 `_lib/financial-extractor.ts` 5종 throw (`equity_current` / `shares_outstanding` / `revenue` / `total_assets` / `shares_outstanding` line 253) 모두 srim Stage 3 호출 chain에서 verdict null 자연 흡수 재확인.

본 ADR 옵션 B 적용 범위는 `_lib/srim-calc.ts` 직접 throw 4건 (line 74·109·115·148)이지만, srim Stage 3 (`_lib/srim.ts`) 안에서 발생하는 financial-extractor throw도 동일 옵션 B 패턴으로 흡수 — `_lib/srim.ts` handler가 try/catch로 throw 잡고 verdict null + note 반환 + 정상 응답.

14단계 (b) 묶음 2 (`feat/stage14-bundle2-data-incomplete`)에서 `_lib/skip-reason.ts` `data_incomplete` 분류 키 추가 — watchlist_check + scan_execute 결과의 skip 분류 정밀화. ADR-0013 옵션 B 동작 변경 0 — 분류 키만 신설.

**상태**: 본 ADR 정합 — null 흡수 동작 변경 없음, 호출 chain 사례 누적.
