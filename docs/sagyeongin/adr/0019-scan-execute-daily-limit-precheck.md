...
### (a) scan_execute 진입 시 scan_preview 자동 호출 + daily_limit_usage_pct > 100 시 throw

scan_execute handler 진입 시 내부적으로 scan_preview 호출 → 응답의 `daily_limit_usage_pct > 100` 검사 → 초과 시 `DailyLimitPreCheckError` throw.
...
**(a) scan_execute 진입 시 scan_preview 자동 호출 + 초과 시 throw 채택.**

### 정책

`scan_execute` handler 진입 직후 (resolved 정합 후):

```typescript
// scan-execute.ts handler
handler: async (ctx, args) => {
  const resolved = resolveInput(args);

  // ADR-0019: daily limit 사전 가드
  const preview = await scanPreviewHandler(ctx, {
    /* 동일 preset/filter */
  });
  if (preview.daily_limit_usage_pct > 100) {
    throw new DailyLimitPreCheckError({
      estimated_calls: preview.estimated_api_calls.total,
      daily_limit: 20000,
      usage_pct: preview.daily_limit_usage_pct,
      message: `Estimated calls (${preview.estimated_api_calls.total}) exceeds daily limit (20000). Narrow universe via included_industries filter.`,
    });
  }
...
- **scan_preview 자동 활용** — 응답 `daily_limit_usage_pct`가 정책 입력

### 트레이드오프

- **scan-execute UX 변경** — 사용자가 included_industries 필터 등으로 universe 축소 후 재진입 1단계 추가
- **scan_preview 자동 호출** — cache miss 시 ~1초 추가 (cache hit 시 ~0)
...
- `src/tools/sagyeongin/scan-preview.ts` (사전 호출 source)
