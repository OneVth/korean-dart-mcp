...
## 부분 개정 (2026-05-31, ADR-0030 — scan 2-모드 게이트)

ADR-0030(scan 2-모드 게이트)이 본 ADR의 사전 차단 동작을 확장한다. **Superseded 아님 — 부분 개정.** 자동 throw 게이트를 버리지 않고 신호 분기로 넓힌다.

### 개정 1 — 옵션 (c) 기각 근거의 전제 정정

위 `## 고려한 옵션`의 (c)(별 도구 신설 + orchestration 사용자 위임)를 "client가 견적 도구를 안 부른다"로 기각했다. 이 기각은 *느슨한 분리*(견적 권장, 강제 없음) 전제였다. *빡센 분리*(견적 없이 실행 불가)는 검토되지 않았다.

ADR-0030 회수로 밝혀진 사실: 빡센 분리를 required 입력으로 만들어도 강제되지 않는다 — client는 required 인자에 임의값을 주입한다. 강제는 **제어 흐름**(신호 부재 시 실행 거부·견적 반환)으로만 가능하며, 도구 형태(분리/단일)와 무관하다. 따라서 (c) 기각 결론(별 도구 분리 불채택)은 유지되나, 근거는 "client가 안 부른다"가 아니라 "분리해도 강제 이점이 없고 복잡도만 증가"로 갱신된다.

### 개정 2 — 결정 (a)의 throw를 신호 분기로 확장

결정 (a) "초과 시 throw"를 ADR-0030이 3-way로 확장한다.

| 조건 | 종전 (본 ADR) | 개정 후 (ADR-0030) |
|---|---|---|
| usagePct ≤ 100 | 완주 | 완주 (불변) |
| 초과 ∧ `allow_over_daily_limit` | 강행 | 강행 (불변) |
| 초과 ∧ `scope_confirmed` | (없던 분기) | 고지 후 완주 (자동 루트) |
| 초과 ∧ 신호 부재 | throw | **견적·분기 구조화 응답 반환 + 사용자 턴**(대화 루트) |

throw(DailyLimitPreCheckError)는 신호 부재 시 견적 반환으로 대체된다. `allow_over_daily_limit` 동작·default 입력의 종전 차단은 불변.

### 비고 — 위 override section의 스키마 기재

위 `### 입력 양식`은 `allow_over_daily_limit: z.boolean().default(false)`로 기재했으나, 실제 구현은 `.optional()` 채택(학습 #58 — `.default()`는 미지정과 명시적 false를 구분 못 해 preset 채택을 깨뜨림). `scope_confirmed`도 동일 사유로 `.optional()`. 본 비고는 회수 확정분 기록이며 override section 본문 정정은 #58이 이미 처리.
