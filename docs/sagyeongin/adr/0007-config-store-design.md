...
ADR-0001~0006으로 디렉토리/브랜치/테스트/순서/커밋/문서가 정해진 후 첫 코드 작업이 시작됐다. 사경인 도구 11개 중 `update_watchlist`, `update_scan_preset` 외에도 `srim`의 `required_return_cache` 갱신, `scan_execute`의 checkpoint, `watchlist_check` 등 거의 모든 도구가 `~/.sagyeongin-dart/config.json`을 읽고 쓴다.
...
- 후속 사용처 (예정): `required-return.ts` (`required_return_cache` 갱신), `srim.ts`, `scan-execute.ts` (checkpoint), `watchlist-check.ts`
