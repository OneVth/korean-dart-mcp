...
- [x] Stage 30.4 scan_preview filter_summary KSIC list 명시 (2026-05-28): feat branch 1 commit `6abbd49` + --no-ff merge `7ca2933` — `buildFilterSummary` 함수 추출 + export (capex-signal 패턴 follow) + `FilterSummary` type export + `filter_summary.excluded_industries` (KSIC list) 신규 + `excluded_industries_count` 유지 (backward-compatible) + spec §10.7 schema 정정 + `scan-preview.test.ts` 신설 (5건). 인지 영역 단독 사이클 (실제 동작 변화 0). 3 files / +83 / -6 net. β-i 가드 정합 (src/lib/ 변경 0). 312/312 test pass. TOOL_REGISTRY 30 (변경 외). Stage 30.4.1 (한도 초과 메시지 노출) 별 사이클 baseline 인계 (TOOL_REGISTRY 30, 사경인 15, 2026-05-28)
- [x] Stage 30.5 user_preference scan_execute induty 통합 (길 A — 목표 축소) (2026-05-28): 검증 로그 main 직접 `2c26e58` (verifications/2026-05-28-stage30.5-precheck-block.md, 48 line, git add -f) + 묶음 1 `c3ddfe2` (`mergeIndustries` pure 함수 — excluded union / included override + 단위 9건) + 묶음 2 `4a708a2` (`resolveInput` loadUserPreference + mergeIndustries 통합 + export + integration 4건, 묶음 1 적층) + --no-ff merge `9677fcc`. 핵심 발견: 원 M2 핵심("universe 축소 → 한도 초과 직접 결판") 미성립 — pre-check(ADR-0019, scan-execute.ts line 716~726)가 name-filter universe로 stage1(induty 필터) 사전 차단 → 한도 경로 merge 무효. 길 A 채택 (sub-limit 실행 반영 단독). 한도 lever = 별 stage (ADR-0010 개정 + pre-check 2-phase 후보). 4 files / +163 / -3 = +160 net. 단테 누적 1277 (실측 재구성). 325/325 test pass. β-i 가드 정합 (src/lib/ 변경 0). 학습 #53 신설 (TOOL_REGISTRY 30, 사경인 15, 2026-05-28)
- [x] Stage 30.4.1 (한도 인지 + 에러 메시지 오안내 정정) (2026-05-29): feat branch 3 commit --no-ff merge `a2af204` — `DailyLimitPreCheckError` 처방형 어구 제거 + 사실 진술 전환 + `scan_preview` `interpretation_notes: string[]` additive 추가 + industry-merge.test.ts noise 정정. 5 files / +49 / -4 = **+45 net**, 단테 누적 **1322**. 329/329 test pass. β-i 가드 정합 (src/lib/ 변경 0). 학습 #54 신설. spec-assumption mismatch 0건. (TOOL_REGISTRY 30, 사경인 15, 2026-05-29)
- [x] Stage 31 ADR-0028 신설 — pre-check 2-phase cache 기반 induty 사전 필터 (2026-05-29): main 직접 3 commit — ADR-0028 신설 + ADR-0010 옵션 D 개정 section + adr/README 인덱스 0028 + verifications baseline (`2026-05-29-stage31-adr-0028-baseline.md`, git add -f) + 본 매듭. 결정 사이클 (code 0). 근본 발견: 한도 잘못된 차단(Stage 30.5)의 해소가 신 인프라 0으로 가능 — corp_meta cache(ADR-0016, induty_code 영구 보유) + corp_meta_refresh(warm 도구) 기존, 빠진 건 pre-check 추정이 cache로 induty 필터를 적용하는 결선 하나. 옵션 B 채택 (cache-hit induty 0 호출 적용 + cache-miss over-estimate). override = ADR-0019 후속 분리. estimated_universe additive 분리 (+after_cache_filter +cache_coverage). 단테 0 (1322 유지). 학습 #55 신설. Stage 32 (구현) 인계. (TOOL_REGISTRY 30, 사경인 15, 2026-05-29)
- [x] Stage 32 ADR-0028 결선 — pre-check 2-phase 구현 (2026-05-29): 묶음 3 사이클 — 묶음 1 `b6aea6a` (필터 함수 _lib 추출 + warm 임계 상수 export, +125/-24 = +101 src/) + 묶음 2 `eaf8080` (splitUniverseByCacheAndFilter helper 신설 + 9 case, +178/-1 = +177 src/) + 묶음 3 `c0e0894` (2 commit 분리 — `d6cad60` spec §10.7 +27/-12 = +15 docs/ + `0b478a2` 코드 결선 +210/-42 = +168 src/) + 본 매듭. Stage 32 src/ 누적 +446 (101+177+168), 단테 누적 1768 (1322 → 1768, 학습 #54 src/ 가산성 정합). 358/358 test pass. β-i 가드 정합 (src/lib/ 변경 0). 학습 #55 직접 적용 (묶음 3 entry 작성 시 ADR-0010 개정 영역 정정 받음 — Stage 31 commit `6f7ea10`에 이미 박힘). 학습 #56 + #57 신설. ADR-0028 B1/B2 결선 완료, B3 (ADR-0010 옵션 D 개정) Stage 31 영역서 이미 박힘. spec §10.7 신 필드 2건 (estimated_universe_after_cache_filter, cache_coverage) additive + interpretation_notes 트리거 2분기 (한도/warm 권고 독립). §10.8 변경 0 (ADR-0019/0028 위임). cleanup 백로그: DailyLimitPreCheckError 메시지 "for more precise estimate" 평가어 영역 정정 (ADR-0025 도구 합리화 거부 baseline). Stage 33 (ADR-0019 후속 override `allow_over_daily_limit`) 인계. (TOOL_REGISTRY 30, 사경인 15, 2026-05-29)
- [x] Stage 33 묶음 1 — override(`allow_over_daily_limit`) 구현 (2026-05-29): feat branch 1 commit `d21cf4e` + --no-ff merge `2cb65ed` — `ScanPreset` + `InputSchema` + `ResolvedInput` + `resolveInput` + pre-check 가드 + `BuildResponseArgs` + `buildResponse`(interpretationNotes + override_applied) + 정상완료 호출부 919 전달. 6 files / +104 / -8 = **+96 net** (+91 src/ + +5 docs/), 단테 누적 **1859**. 364/364 test pass. β-i 가드 정합 (src/lib/ 변경 0). `scan-execute.test.ts` 신설 (buildResponse 2건) + `scan-execute-resolveinput.test.ts` +4건. spec §10.8 additive (input/pipeline_stats/output + override 문단). 핵심 결정: `z.boolean().optional()` 채택 — `.default(false)` 시 Zod가 false를 주입해 `??` 연산자가 preset 채택 불가 → resolveInput 층에서 `?? false` 기본값 채움(학습 #57 직접 적용). cache-miss 부정확성 진술 보류 (buildResponse에 split 정보 미보유, override 시 추정 무의미 — ADR-0025 합리화 거부 정합). (TOOL_REGISTRY 30, 사경인 15, 2026-05-29)
- [x] cleanup — DailyLimitPreCheckError 평가어 정정 (2026-05-29): fix branch 1 commit + --no-ff merge `9e17016` — 메시지 마지막 줄 `for more precise estimate` 평가어 제거 → 사실 진술 `apply induty filter to cache-miss companies in the estimate` (scan_preview buildLimitNotes b 분기 동형, 영문 유지). 회귀 가드 assert 2건 추가 (`precise|better|optimal` 부재 + `apply induty filter` 존재). 2 files / +4 / -1 = **+3 net** (src/ only), 단테 누적 **1862**. 364/364 test pass. β-i 가드 정합 (src/lib/ 변경 0). Stage 32 인계 cleanup 백로그 종결. ADR/spec 변경 0 (ADR-0025 기존 baseline). (TOOL_REGISTRY 30, 사경인 15, 2026-05-29)
- [x] Stage 30.7 — composite_score 산식 재설계 + min_opportunity_score 안내 보강 (2026-05-30): 묶음 2 사이클 + 매듭 — 묶음 1 `828804e` (`SRIM_GAP_WEIGHT=1.5` 상수 + finalizeCandidates 산식 교체 + 테스트 6건) + 묶음 2 `ba2ae98` (InputSchema `.describe()` 5필드 + description 1문장) + --no-ff merge 2건 + 본 매듭. `composite = gap × 1.5 + opportunity − concern` (ADR-0029). 7부 D 정렬 주도 편입 — capex 희소 0-tie 사태 해소. src/ 2 files / +122/-11 = **+111 net**, 단테 누적 **1973** (baseline 1862 + 111, `git diff --stat origin/main..HEAD -- src/` 실측). spec §10.8/§7.1 산식 갱신 + spec-pending-edits entry 5 종결 + ADR-0029 신설 + adr/README 인덱스 추가. 학습 #59 신설. β-i 가드 정합 (src/lib/ + SIGNAL_SCORES 무변경). (TOOL_REGISTRY 30, 사경인 15, 2026-05-30)
- [x] Stage 30.7-fix — 옛 finalize 테스트 정정 + 일원화 (2026-05-30): main 직접 1 commit — scan-enrich.test.ts 옛 finalize 블록 삭제 (5건 + makeEnriched helper) + import 정리 (finalizeCandidates/ResolvedInput) + scan-execute.test.ts 필터/limit 케이스 2건 이전 (새 산식 기준). 테스트 370 → 367 (삭제 5 + 추가 2). 2 fail → 0 fail. 코드 무변경 (scan-execute.ts 본문 변경 0). 학습 #60 신설. β-i 가드 정합. (TOOL_REGISTRY 30, 사경인 15, 단테 1973, 2026-05-30)
- [x] Stage 30.7-fix2 — composite_score 산식 부호 정정 (ADR-0029 개정, 2026-05-30): gap_to_fair 괴리율(쌀수록 음수) 반영 → discount = max(−gap, 0). 테스트 367→368 (고평가 회귀 가드 +1). src/ 2 files / +30/-23 = **+7 net**, 단테 누적 **1980** (baseline 1973 + 7). 학습 #61 신설. β-i 가드 정합 (srim-calc.ts·src/lib/ 무변경). (TOOL_REGISTRY 30, 사경인 15, 단테 1980, 2026-05-30)
- [x] Stage 30.7 field-test 종결 (2026-05-30): main 직접 — ADR-0029 Status 갱신(재스캔 n=16 field-test 결과 + SRIM_GAP_WEIGHT 잠정 유지·재판정 보류) + 학습 #62 신설(push 검증 자가확인 강제). 실측: gap 전원 음수(min −89, avg −46%), composite 전원 양수, 부호 정정 확인. capex opportunity_score 전원 0(공시 희소) — 1.5 가중치 capex-vs-gap 균형 기능 미발동, 재판정 보류. 코드 변경 0, 단테 1980 유지. 학습 #62 신설. (TOOL_REGISTRY 30, 사경인 15, 단테 1980, 2026-05-30)
- [x] Stage 30.8 — preset/filter 적용 사실 전달 유도 (피드백 2, 2026-05-30): feat branch `--no-ff` merge `f3c7d47` — sagyeongin_scan_execute 응답에 `preset_used` + `filter_summary` 추가(preview와 동형) + description 양쪽 유도(execute 본체 + preview 보강, excluded_industries·markets 자연어 설명 명령 + blacklist 합산 전달). `buildFilterSummary(resolved)` 재사용 — ResolvedInput에 `preset_used` 필드 추가. 호출부 5곳 전달(정상 + limit 2 + saveAndReturnPartial 시그니처 resolved 추가). src/ 4 files / +58/−1 = **+57 net**, 단테 누적 **2037**. 371/371 test pass(신규 3건: resolveInput preset_used + buildResponse 키/구조 2건). β-i 가드 정합 (src/lib/·산식·필터 무변경). 학습 #63 신설. 결함 데이터 측면 해소 — client 자연어 풀이는 코드 검증 불가(C-2 한계), field-test 잔존. (TOOL_REGISTRY 30, 사경인 15, 단테 2037, 2026-05-30)

### 현재 작업 단계

Stage 30.8 종결 (2026-05-30). 피드백 2(preset 적용 전달 누락) 데이터 측면 해소 — scan_execute 출력에 preset_used·filter_summary 추가 + description 유도. 단테 2037.

인계: 다음 단계 = Stage 30.6 (default preset redesign — 일반 사용자 영역 일치). 진입 baseline = 단테 2037, origin/main Stage 30.8 docs 매듭 hash. 잔존: Stage 30.8 field-test (client가 제외 업종·시장을 사용자에게 자연어로 푸는지 실사용 확인 — C-2 코드 검증 불가분).
...
#### Stage 21 phase 1 누적 (2026-05-17)

본 사이클 phase 1에서 정착된 3건 누적 학습:

27. **사이클 본질 entry prompt 가정 위반 — origin 본문 자체 모순 식별 정합** — entry prompt 분기 3건 가정 (분기 1 K_floor / 분기 2 ROE_cap / 분기 3 fair_buy_invariant 가드) 모두 ROE > K 큰 격차 전제. 회수 D (18(iii) results 직접 view) + 회수 A (`_lib/srim-calc.ts` line 119-132 multiplier 산출 본문 직접 view)에서 실제 본질 식별 — ROE < K 케이스 분포 역전 (9/10 baseline). origin "고ROE × K 보정" 영역 자체 부정확. 학습 21 ("호출 입력 schema 직접 회수 가드") 본문 본질 확장 — schema 외 *공식/계산 본문* 직접 회수 정합. **반복 금지.**

28. **srim 공식 분포 ROE/K 의존 본질** — `_lib/srim-calc.ts` line 121-123 W08/W09/W10 산출 본문에서 분모 (1+K-W)가 W=1.0에서 최소. excess income 부호에 따라 분포 결정:
    - ROE > K (excess 양수) → sell > fair > buy (안전마진 정합)
    - ROE = K (excess 0) → buy = fair = sell (가격 압축)
    - ROE < K (excess 음수) → buy > fair > sell (분포 역전)
...
#### Stage 30.1 (γ 코드 정착 사이클) — ADR-0027 코드 정착 + null 흡수 정책 (2026-05-24)

##### 본질

Stage 30.0 결정 (ADR-0027 + spec §10.16) 코드 정착. judgeExistingBusinessMatch signature 정밀화 + classifySignal null 흡수 정책 정착.

##### commit chain

phase 1 (`feat/stage30.1-phase1-judge-existing-business-match`, HEAD `5ef510b`, 4 commit):
- `f2d95cc` — `_lib/existing-business-keywords.ts` 신설 (matcher 3건 export)
- `97d87a1` — judgeExistingBusinessMatch 본문 정밀화 (boolean | null + matcher chain)
- `bfebba2` — matcher 단위 테스트 21건
- `5ef510b` — 회수 F 13건 regression + corner case 5건

phase 2 (`feat/stage30.1-phase2-classify-signal-null-absorb`, HEAD `bb81ff0`, 4 commit):
- `5f37773` — ADR-0027 정정 (signature 3→2 params + blacklist 임대 + §결과 null 흡수 명시)
- `6fb2560` — classifySignal 호출부 정정 (ternary → if-else strict check)
- `b6fd0da` — signalName 분기 테스트 (null/false/true + case 11/12)
- `bb81ff0` — phase 2 summary 보존 (`verifications/stage30.1/`)

phase 3 (main 직접, 2 commit):
- merge commit (`--no-ff`)
- 본 매듭 commit (CLAUDE.md sub-section + 학습 #46 신설)

##### 결판 본문

| 영역 | 정착 본문 |
|---|---|
| signature | `judgeExistingBusinessMatch(text: string, _induty_code?: string): boolean \| null` |
| 분기 chain | blacklist → mixed (whitelist + null pattern) → whitelist → null pattern → default null |
| 호출부 null 흡수 | `existingMatch === false` strict check. false → unrelated_diversification, null/true → existing_business (7부 C 긍정 분기) |
| keyword 모듈 | `_lib/existing-business-keywords.ts` (matcher 함수 3건 export, keyword internal) |
...
29 유지 (사경인 14). 본 사이클 도구 추가 없음 — 기존 `capex_signal` 정밀화.

##### field-test 인계

본 사이클 외. 별 사이클 (Stage 30.1.1 또는 `verifications/stage30.1-field-test` branch).

검증 대상:
- 인콘(468) `capex_signal` → signalName = `major_capex_existing_business` (case 11 mixed/null 흡수 정합)
- 한화리츠(68112) `capex_signal` → signalName = `major_capex_unrelated_diversification` (case 12 blacklist 임대 정합)
- 회수 F 13건 전수 실제 호출 + signalName 분포

환경: 한국 IP + DART API 키 + MCP 호출.
...
**통합 단테**: 6 files / +395 / -1 = **+394 net** (실측 소급 정정, Stage 30.4.1 매듭), 누적 **975** (30.1 종점 581 + 394).
...
#### Stage 30.3 (ε MVP M1 사후 — cache pre-check 정정 사이클) — `estimateApiCalls` cacheHitCount 정착 (2026-05-27)

**Stage 30.2 baseline 인계** — M1 (`sagyeongin_user_preference`) 정착 `b45e66f` + 매듭 `2e9235d` 사후 본 사이클 진입. M1 실측 회수 사후 사이클 4건 분리 baseline 중 우선 진입 사이클.

**사이클 본질**: pre-check (ADR-0019) 산식 정정 — `estimateApiCalls` stage1 영역 corp_meta cache (ADR-0016) 적중분 차감 정착. cache hit 무시 → 과대추정 → 사용자 호출 차단 패턴 해소.

**묶음 1** (commit `e55542e`):
- `src/tools/sagyeongin/_lib/scan-helpers.ts` — `estimateApiCalls` signature positional 3-arg → opts object 정정
  - `opts.cacheHitCount` → `Math.min(cacheHitCount, universeCount)` 가드 + stage1 effective universe 차감 (stage2~6 불변)
  - opts undefined → cacheHits = 0 → 기존 산식 정합 (호출부 무변경 동작 baseline)
- `_lib/corp-meta-cache.ts` — `corpMetaSizeForUniverse(corpCodes: string[]): number` export 신설
  - IN 절 prepared statement + 빈 배열 가드 (`corpCodes.length === 0 → return 0`)
- 단위 테스트 — `scan-helpers.test.ts` case 4 opts 양식 정정 + cacheHitCount test 1건 신설 / `corp-meta-cache.test.ts` 2건 신설 (빈 배열 가드 + 3건 저장 universe 2건 포함)

**묶음 2** (commit `b383cbe`):
- `src/tools/sagyeongin/scan-preview.ts:91` — `estimateApiCalls(estimatedUniverse, { cacheHitCount: corpMetaSizeForUniverse(filtered.map(c => c.corp_code)) })` 정정 + import 추가
- `src/tools/sagyeongin/scan-execute.ts:718` — 동일 패턴 + import 추가
- `src/tools/sagyeongin/scan-execute-precheck.test.ts` — ADR-0016 cache hit case 신설 (universe 3607 + cacheHitCount 3500 → stage1 = 107)

**머지**: `feat/stage-30-3-cache-precheck` `--no-ff` merge `cd58190` → origin/main 통합.

**통합 단테**: 7 files / +74 / -8 = **+66 net** (`src/lib/` 변경 0 — β-i 가드 정합), 누적 **1041**.

**테스트**: 전체 `test:unit` 307/307 pass (기존 302 + 신규 5건). regression 외.

**TOOL_REGISTRY**: 30 (변경 외 — 인프라 정정 단독).

**push 누락 패턴 식별 2건**:
- 묶음 1 case C (main 직접 commit) — `git reset --hard origin/main` + feature branch 재정착 + cherry-pick 사후 push 정상화
- 묶음 2 case C (uncommitted) — Claude Code 동일 세션 commit + push 정상화

**M2 결판 baseline 인계**: Stage 30.3 정정 사후 한도 초과 (stage2~6 cache 외 ≈29,000건) 잔존 — Stage 30.5 (M2 induty 필터) 진입 baseline 우선. Stage 30.4 (default preset 응답 본문 명시) 단축 사이클 우선 진입.

##### 학습 #51 — push 누락 패턴 정착 (Claude Code 위임 영역)

**baseline**: Stage 30.3 사이클 영역 push 누락 case 2건 발생 — 묶음 1 main 직접 commit + 묶음 2 uncommitted. Claude 직접 검증 (origin fresh fetch) 사후 식별.

**본질**: Claude Code 본 실행 보고 본문 = 사후 commit/push 정합 보장 외. 위임 명세 영역 push 강제 step + 보고 양식 commit hash 본문 명시 강제 패턴 baseline.

**적용**:
1. 위임 명세 영역 본 실행 단계 끝 `git push origin <branch>` 명시 강제
2. 보고 양식 영역 commit hash + origin HEAD 본문 명시 강제 (보고 본문 "완료" 단독 외)
3. case 식별 영역 (A 정상 / B main 직접 / C uncommitted / D 별 branch) push 검증 영역 확정 baseline
4. push 누락 식별 사후 사용자 (Onev) 결판 회수 — case B 재발 시 c-1 (feature branch 재정착) baseline

**Ref**: Stage 30.3 묶음 1 case C + 묶음 2 case C 식별 사이클.

##### 학습 #52 — Claude 직접 clone 검증 baseline (Claude Code 보고 trust 외)

**baseline**: Stage 30.3 사이클 영역 Claude 직접 `/tmp` clone fresh fetch 검증 5건 (묶음 1 사후 / 묶음 1 push / 묶음 2 사후 / 묶음 2 push / merge 사후) 정합. 학습 #51 push 누락 패턴 식별 baseline 본 영역 정합.

**본질**: Claude Code 보고 본문 = trust 외 — Claude 영역 origin/main 직접 fetch + diff view 강제. 학습 #46 (commit 메시지 vs 코드 mismatch) 정합 확장 — push 정합 검증 단독 본 영역 정착.

**적용**:
1. Claude Code 본 실행 / push 보고 사후 Claude 직접 `git fetch origin` + `git log` + `git diff --stat <baseline>..<HEAD>` 검증 강제
2. β-i 가드 (`src/lib/` 변경 0) 직접 verify
3. 단테 영역 실측 정합 (학습 #47) 직접 verify
4. commit message 본문 (Conventional + 한국어 + Ref trailer) 직접 verify
5. 어긋남 식별 사후 정정 baseline 또는 매듭 영역 보강 baseline 결판

**Ref**: Stage 30.3 사이클 영역 5건 직접 검증 정합 (묶음 1 6건 ✓ + 묶음 2 7건 ✓ + merge 5건 ✓).

##### 학습 #53 — 상류 게이트 호출부 회수 누락 (merge 대상 단독 view 한계)

**baseline**: Stage 30.5 원 entry prompt 작성 영역 — `resolveInput`(merge 대상) 본문은 회수했으나 그 산출을 게이팅하는 상류 pre-check 호출부(scan-execute.ts line 716~726) 회수 누락 → "resolveInput merge → universe 축소 → 한도 초과 결판" 가정 어긋남. pre-check가 merge 산출(included/excluded_industries) 미소비 (name pattern 단독) → 한도 경로 merge 무효.

**핵심**: 학습 #44("호출부 raw")를 merge 대상엔 적용했으나, 대상 산출이 흐르기 전 그것을 차단·변형하는 **상류 게이트**까지 회수 안 함. 단일 함수 본문 일치 ≠ 동작 일치.

**적용**:
1. merge·정정 대상 식별 시 4종 raw 회수 — (a) 대상 본문 (b) 대상 호출부 (c) 대상 산출 소비처 (d) 대상 산출이 소비처에 도달하기 전 게이팅하는 상류 분기 (pre-check / early return / throw).
2. "X 통합 → Y 효과" 가정 시 X 산출이 Y 경로에 실제 도달하는지 호출 순서 직접 verify (게이트 사전 차단 식별).
3. 발견 = verifications artifact 정착 (학습 #45).

**Ref**: Stage 30.5 검증 (pre-check 차단 발견), verifications/2026-05-28-stage30.5-precheck-block.md, 학습 #44 보강. **반복 금지.**

##### 학습 #54 — 단테 stage별 src/ delta 가산성

**baseline**: Stage 30.4.1 매듭 소급 정정 — 30.2~30.5 개별 delta(394+66+76+160=696)가 end-to-end range(`19cf035..9677fcc` src/ +714/-18 = net 696)와 정합.

**본질**: 단테 누적 = stage별 delta 누적 합. **동일 line 교차 수정 부재 시** delta 합 = end-to-end net (본 range 696=696 실측). "reviewer 비가산 진단"은 Stage 30.4 delta 77→76 오측 산물 — 실측 76(+82/-6) 정합.

**소급 정정 근거**:
- Stage 30.2: CLAUDE.md 누락 → 실측 +394 net (6 files / +395/-1)
- Stage 30.3: CLAUDE.md "+66 net" ✓ (cumulative 1041 미기재)
- Stage 30.4: CLAUDE.md "+77 net" → **+76 net** 정정 (실측 2 files / +82/-6)
- Stage 30.5 재구성 이미 반영 ("+714/-18 = net 696" cross-check)

**적용**: 단테 누적 재구성 필요 시 (a) end-to-end `git diff --stat <base>..<merge> -- src/` → 전체 확인 (b) stage별 delta 합 비교 (c) 불일치 stage 개별 실측 정정. **반복 금지.**

**Ref**: Stage 30.4.1 매듭, 학습 #47 (단테 누적 본문 실측 정합 본질).

##### 학습 #55 — 근본 lever 발견 — 기존 자산 인벤토리 우선

**baseline**: Stage 31 — 한도 차단 해소가 신 캐시·도구 없이 가능했음 (corp_meta cache + warm 도구가 닭-달걀을 이미 깨고 있었고 빠진 건 결선뿐). ADR 작성 전 "기존 자산이 이미 푸는가" 가드.

**본질**: 차단/제약 문제는 신 인프라 추가 전에 기존 인프라가 전제를 이미 충족하는지 먼저 확인. 신 코드·도구 설계는 기존 자산 인벤토리 소진 후.

**적용**: 차단/제약 근본 해소 ADR 작성 시 — (a) 해소에 필요한 전제 목록화 (b) 기존 자산(cache/도구/함수) 대조 (c) 빠진 것이 결선만인지, 실제 신 인프라 필요인지 구분. 신 인프라 추가는 (c) 확인 후.

**Ref**: Stage 31 baseline 검증 (verifications/2026-05-29-stage31-adr-0028-baseline.md), ADR-0028 옵션 B 채택 근거. **반복 금지.**

#### Stage 30.4 (ε MVP M1 사후 — default preset 본문 명시 사이클) — `scan_preview` filter_summary KSIC list 노출 (2026-05-28)

**Stage 30.3 baseline 인계** — cache pre-check 정정 사후 사이클 4건 분리 baseline 중 두 번째 진입 사이클.

**사이클 핵심**: 인지 영역 단독 정착 사이클 — 실제 동작 변화 0. `scan_preview` 응답 `filter_summary` 영역 default preset KSIC list (27건) 사용자 인지 노출 → 7부 A "즉시 솎아내기" 간접 정합.

**묶음 1** (commit `6abbd49`):
- `src/tools/sagyeongin/scan-preview.ts` — `buildFilterSummary` 함수 추출 + export (capex-signal 패턴 follow) + `FilterSummary` type export 신설
- `filter_summary` 영역 `excluded_industries` (KSIC list) 신규 + `excluded_industries_count` 유지 (backward-compatible)
- `src/tools/sagyeongin/scan-preview.test.ts` 신설 — 5 case
- `docs/sagyeongin/sagyeongin-dart-agent-spec.md` § 10.7 schema 정정 (1줄 추가)

**머지**: `feat/stage-30-4-default-preset` `--no-ff` merge `7ca2933` → origin/main 통합.

**통합 단테**: 2 files / +82 / -6 = **+76 net** (소급 정정; 이전 "+77" = +83/-6 오측, 실측 +82/-6), 누적 **1117**.

**테스트**: 전체 `test:unit` 312/312 pass (기존 307 + 신규 5건). regression 외.

**TOOL_REGISTRY**: 30 (변경 외 — 응답 양식 정정 단독).

**Stage 30.4.1 인계 (식별 6번 별 사이클 baseline)**:

식별 baseline: `scan_preview` + `scan_execute` 응답 영역 `daily_limit_usage_pct ≥ 100` 본문 명시 메시지 없음 — 한도 초과 사용자 인지 약함.

본 사이클 영역 외 (Stage 30.4 = `default preset 본문 명시` 단독 영역). 별 사이클 baseline 인계 — Stage 30.4.1 영역 진입 결판 baseline (사용자 결판).

**M2 결판 baseline 인계**: Stage 30.4 정착 사후 Stage 30.5 (M2 induty 필터) 진입 baseline 우선 — universe 축소 → 한도 초과 직접 결판.

#### Stage 30.5 (ε MVP M2 사이클 — 목표 축소) — user_preference scan_execute induty 통합 (2026-05-28)

**Stage 30.4 baseline 인계** — M1 실측 회수 사후 사이클 4건 분리 중 M2 진입. 원 핵심 = "universe 축소 → 한도 초과 직접 결판". 검증으로 미성립 확정 → 길 A (목표 축소) 채택.

**핵심 발견 (scope 축소 근거)**: `resolveInput` user_preference merge는 `stage1StaticFilter`(induty 필터, line 315~324) 사후 적용분만 바꾼다. scan_execute pre-check(ADR-0019, scan-execute.ts line 716~726)가 그 사전에 `filterUniverse`(name pattern 단독)로 universe를 세어 차단 → 한도 초과 시 stage1 도달 불가 → merge 코드 dead. cold cache 시 induty_code(company.json 호출 사후만 회수) 무지 = ADR-0010 over-estimate. 부수: 에러 메시지(line 89) "Narrow universe via included_industries filter" 오안내 (pre-check 미반영). 검증 로그: verifications/2026-05-28-stage30.5-precheck-block.md.

**길 A 결판**: Stage 30.5 = "user_preference 실 실행(stage1StaticFilter) 반영" 단독. pre-check 통과(sub-limit) universe에서 induty 솎아내기 작동. 한도 초과 lever = 별 stage (ADR-0010 개정 + (1) pre-check 2-phase 분할 — stage1 선행 cache 적재 + induty 축소 → stage2+ 재추정, cold cache 실효, CHECKPOINT_THRESHOLD 단서 / (2) cache warm-up 명령 — (1) 우선).

**검증 로그 commit** (main 직접 `2c26e58`): verifications/2026-05-28-stage30.5-precheck-block.md (48 line, git add -f — 학습 #45).

**묶음 1** (`feat/stage30.5-merge-industries` `c3ddfe2`):
- `src/tools/sagyeongin/_lib/industry-merge.ts` 신설 — `mergeIndustries(presetList, prefList, strategy)` pure 함수. 비대칭: excluded union (솎아내기 강화) / included override (whitelist 우선, intersection 공집합 배제). 빈 결과 undefined.
- scan-helpers.ts("8단계 / induty 분기 0" 불변식) 부적합 → 신규 단독 모듈.
- 단위 테스트 9건 (union 5 + override 4).

**묶음 2** (`feat/stage30.5-resolveinput-userpref` `4a708a2`, 묶음 1 적층):
- `scan-execute.ts` — import 2줄 + `resolveInput` export + `loadUserPreference` + `mergeIndustries` 비대칭 통합 (included override / excluded union). 변경 import + resolveInput 한정 (다른 함수 무변경).
- integration 테스트 신설 (`scan-execute-resolveinput.test.ts`) — config.json + user-preference.json mock (SAGYEONGIN_CONFIG_DIR), resolveInput 직접 호출 (pre-check 우회). 4건 (merge / empty pref / args 우선 / whitelist 빈).

**머지**: `--no-ff` merge `9677fcc` (2-parent 2c26e58 + 4a708a2).

**통합 단테**: 4 files / +163 / -3 = **+160 net**. 누적 = 581(30.1 종점) + 696(30.2~30.5 실측, `19cf035..9677fcc` src/ = +714/-18) = **1277**. ※ 30.2 subsection 단테 누락 + 30.3/30.4 누적 미재기재 → 본 매듭 실측 재구성. 30.2~30.4 subsection 누적 line 소급 정정은 cleanup 백로그.

**테스트**: 325/325 pass.

**TOOL_REGISTRY**: 30 유지 (사경인 15). 도구 추가 없음 — resolveInput 통합 단독.

**spec-assumption mismatch**: 14+ → 15+ (원 entry prompt "resolveInput merge → universe 축소" 가정 어긋남 — pre-check 사전 차단 미식별).

**인계**:
- Stage 30.4.1 (한도 메시지 + 에러 메시지 line 89 오안내 정정) — calibration Q2 (β) 우선.
- Stage 30.6 (default preset redesign — 일반 사용자 영역).
- 한도 초과 lever 별 stage (ADR-0010 개정 + pre-check 2-phase 분할 우선).
- cleanup 백로그: industry-merge.test.ts noise 정정 ✓ (Stage 30.4.1 commit 3) + 30.2~30.4 단테 누적 소급 ✓ (Stage 30.4.1 매듭, 학습 #54 정합).

#### Stage 30.4.1 (한도 인지 + 에러 메시지 오안내 정정) (2026-05-29)

**Stage 30.5 baseline 인계** — pre-check 에러 메시지 오안내 정정 + scan_preview 한도 인지 노출 사이클.

**핵심 발견 (Stage 30.5 인계)**: `DailyLimitPreCheckError` 메시지 "Narrow universe via included_industries filter"가 오안내 — pre-check는 `excluded_name_patterns` 단독 기반, `induty/included_industries`는 한도 안쪽 실행에만 적용. 에러 메시지가 이를 반영하지 않음 + `scan_preview`도 한도 초과 시 사용자 인지 수단 부재.

**commit chain** (`feat/stage-30-4-1-limit-awareness`, 3 commit → `--no-ff` merge `a2af204`):
- `64dacbc` commit 1 — `DailyLimitPreCheckError` `super()` 처방형 어구 제거 + 사실 진술 전환 (`exceed` 단수 + pre-check/within-limit 분리 명기) + precheck test 3 assert 추가 (`excluded_name_patterns only` / `within-limit execution` / `doesNotMatch Narrow universe`)
- `9956be2` commit 2 — `scan-preview.ts` `buildLimitNotes` pure fn 추출 + `DAILY_LIMIT` import + handler `interpretationNotes` + return `interpretation_notes: string[]` additive 추가 + test 4건 (163.2% / 100% 경계 / 100.1% 직상 / 50% 정상)
- `1337745` commit 3 — `industry-merge.test.ts` line 4 주석 `"영역 본질:"` → `"케이스:"` (noise 정정, cleanup 백로그 완결)

**통합 단테**: 5 files / +49 / -4 = **+45 net** (src/ 실측), 누적 **1322** (Stage 30.5 1277 + 45).

**테스트**: 329/329 pass (기존 325 + 신규 7건: precheck +3, scan-preview +4).

**TOOL_REGISTRY**: 30 유지 (사경인 15). 도구 추가 없음 — 응답 양식 + 에러 메시지 정정 단독.

**spec-assumption mismatch**: 15+ 유지 (본 사이클 0건 — spec Q1~Q4 7부 E 판단 정합).

**인계**:
- Stage 30.6 (default preset redesign — 일반 사용자 영역).
- 한도 초과 lever 별 stage (ADR-0010 개정 + pre-check 2-phase 분할 우선).

#### Stage 31 — ADR-0028 신설 (pre-check 2-phase) (2026-05-29)

본 사이클 핵심 — Stage 30.5에서 확정된 한도 잘못된 차단의 근본 해소 결정. induty 필터가 pre-check 추정에 반영되지 않아(scan-execute.ts 729~739, name-filter universe 단독) induty로 universe를 줄여도 차단이 그대로였던 문제.

근본 발견: 해소에 필요한 전제가 이미 존재 — corp_meta cache(ADR-0016)가 induty_code를 영구 보유하고 corp_meta_refresh 도구가 cache를 채운다. 빠진 것은 pre-check 추정이 cache를 보고 induty 필터를 적용하는 결선 하나. 신 인프라 0.

결정 (옵션 B): name-filter universe를 cache로 분할 — cache-hit ∧ market/induty 통과분(H')은 0 호출 적용, cache-miss(M)는 보수적 전부 통과 가정. estimateApiCalls(H'+M, {cacheHitCount: H'}) — 시그니처 불변. 응답은 estimated_universe(불변) + estimated_universe_after_cache_filter(실 기준) + cache_coverage(신뢰도) additive 분리. override는 성격이 반대(한도 무시)라 ADR-0019 후속으로 분리. ADR-0010 옵션 D over-estimate 전제는 개정 section으로 갱신.

산출 (main 직접 3 commit): ADR-0028 + ADR-0010 개정 + adr/README 인덱스 / verifications baseline (git add -f) / 본 매듭. code 0, 단테 0 (1322 유지). 학습 #55 신설.

#### Stage 32 — ADR-0028 결선 (pre-check 2-phase 구현) (2026-05-29)

본 사이클 핵심 — Stage 31에서 결정된 ADR-0028 옵션 B를 코드로 박는 구현 사이클. 묶음 3개 분리 (필터 추출 → helper 신설 → 결선).

진행 양식: 묶음별 사전 anchor 회수(학습 #44/#49) → 위임 명세 md 산출 → Claude Code 실행 → Claude 직접 fresh clone 검증(학습 #52, 보고 trust 외) → main `--no-ff` 머지 → 다음 묶음 baseline 갱신. 매 묶음 동일 패턴 3회 반복.

결과:
- **묶음 1** `b6aea6a` (refactor 사이클): `scan-execute.ts` module-private `isMarketMatch`/`isIndustryMatch` → `_lib/scan-helpers.ts` export 이전 + `CACHE_COVERAGE_WARM_THRESHOLD_PCT = 50` 상수 신설 + 단위 테스트 14+1 case. 행동 무변경 baseline.
- **묶음 2** `eaf8080` (feat 사이클): `splitUniverseByCacheAndFilter` helper 신설 — universe → {matched_cached_count: H', cache_miss_count: M} 분할. 단위 테스트 9 case (corp-meta-cache.test.ts tmpDir setup follow). 외부 호출자 0 — 묶음 3에서 결선.
- **묶음 3** `c0e0894` (결선 사이클, 2 commit 분리): commit 3a `d6cad60` `docs(spec)` §10.7 갱신 (신 필드 2건 `estimated_universe_after_cache_filter` + `cache_coverage` additive + interpretation_notes 트리거 2분기 한도/warm 명시) + commit 3b `0b478a2` `feat(sagyeongin)` 코드 결선 (scan-execute pre-check + scan-preview handler + buildLimitNotes 시그니처 변경 + DailyLimitPreCheckError 메시지 본문 갱신 + 기존 case 정정 + 신 6 case). `corpMetaSizeForUniverse` 호출 0 (splitUniverse 흡수).

본 사이클서 학습 #55 직접 적용 — 묶음 3 entry 작성 시 ADR-0010 개정을 묶음 3 spec 변경에 포함하려 함 → 다른 세션 검토에서 Stage 31 commit `6f7ea10`에 이미 박힘 회수 → 묶음 3에서 ADR-0010 제외 정정. 학습 #55 가드 정확 작동.

단테 가산성 (학습 #54 정합): baseline 1322 (Stage 31 종결) + 묶음 1 +101 + 묶음 2 +177 + 묶음 3 +168 (src/ only) = **1768**. spec md +15는 docs/ 영역 별 분리. 본 사이클서 누적 1322 + 168 = 1490 회수 발생 → 묶음 1/2 src/ net 가산 누락 정정 → 학습 #57 신설 직접 baseline.

산출 (feat branch 3개 → `--no-ff` 머지 3개 + 본 매듭 main 직접 single, 학습 #41 정합).

**학습 #56 신설** — spec docs commit과 code commit 분리 baseline은 ADR-0005 commit scope (`docs(spec):` ≠ `feat(sagyeongin):`) + 학습 #54 단테 가산성(src/ vs docs/) 자연 귀결. ADR-0006은 documentation strategy 영역 — "spec 먼저, 코드 나중" 직접 명시 부재. 검토자 영역 회수 시 ADR 번호 회수 부정확 가능, 원칙 본질 영역 정정으로 회수. 본 사이클 묶음 3 영역서 ADR-0006 인용 받음 → 실측 후 ADR-0005 + #54 자연 귀결로 정정.

**학습 #57 신설** — 단테 누적 baseline은 Stage N-1 매듭 commit hash 종결 시점 영역. 신 사이클 묶음별 src/ delta 모두 baseline에 가산 — 본 묶음 단독 영역 아님. 매듭 commit 본문 영역에 `baseline (Stage N-1 commit hash) + 묶음별 src/ net + 누적 합` 3 영역 명시 가드. 학습 #54 가산성 적용 영역. 본 사이클서 누적 1322 + 168 = 1490 회수 발생 → 묶음 1/2 net 가산 누락 정정 (정확 1768) → 본 학습 신설 baseline.

ADR-0028 결선 영역 완료 결판:
- B1 (응답 양식 additive 분리) ✓ — 묶음 3 commit 3a/3b
- B2 (cache-miss 보수 처리 + warm 안내) ✓ — `CACHE_COVERAGE_WARM_THRESHOLD_PCT = 50` + buildLimitNotes 신 분기
- B3 (ADR-0010 옵션 D 개정) ✓ — Stage 31 commit `6f7ea10` 영역서 이미 박힘

cleanup 백로그 (별 사이클): DailyLimitPreCheckError 메시지 "for more precise estimate" 평가어 영역 정정. ADR-0025 도구 합리화 거부 baseline 정합 — `precise estimate` 같은 평가어는 사실 진술이 아닌 도구 평가 영역. Q2 정정안 "정밀화 같은 평가어 빼라" 가드 영역 누락. spec-pending-edits.md 누적 또는 별 정정 사이클 영역.
...
#### Stage 30.7 누적 (2026-05-30)

59. **composite_score 산식 설계 원칙 — 7부 D 정렬 주도 + 7부 C tie-breaker 위계** — 산식 설계 시 7부 철학 위계를 산식 구조에 직접 투영: (1) 탈락 기준(7부 A/D-2)은 Stage gate로, (2) 정렬 주도(7부 D = "얼마나 싼가")는 가중치가 가장 큰 항으로, (3) 가점(7부 C/B/E)은 보조 항으로. "정보 가용성"(capex 희소)이 "설계 의도"(7부 D 주도)를 바꿔서는 안 된다. capex 희소 → opportunity_score=0 → 0-tie는 데이터 한계가 아니라 산식 설계 결함. 가중치(`SRIM_GAP_WEIGHT`)는 임의 숫자 주입 금지 — field-test 갭 분포 실측 후 조정, ADR에 근거 기록.

60. **공유 함수 테스트 분산 가드 — spec 0단계 전수 grep** — 공유 함수(finalizeCandidates 등) 단위 테스트가 복수 파일에 분산될 수 있다. 산식/로직 변경 시 spec 0단계에서 변경 대상 함수명으로 `grep -rn "<fn>" src/` 전수 확인 → 모든 검증 파일을 spec 범위에 포함. (Stage 30.7: scan-execute.test.ts만 상정, scan-enrich.test.ts 옛 finalize 블록 누락 → 2 fail.)

61. **변수 의미론(부호 규약·단위) raw 회수 강제** — 산식에 외부 변수 투입 시, 그 변수의 *계산식*을 직접 view해 부호·단위·범위를 확인. scan-execute 매핑만 보고 gap_to_fair를 "저평가 폭(양수)"로 가정했으나 실제 `_lib/srim-calc.ts:180`은 괴리율(쌀수록 음수). field-test 실데이터(전원 음수)로만 드러남. #44 raw 회수에 "변수 정의식까지" 추가. **반복 금지.**

62. **Claude Code 보고 양식에 push 검증 자가확인 강제** — 매듭/정정 commit 후 `git log -1 origin/main` 실행 결과(해시)를 보고에 그대로 붙이고, 방금 commit 해시와 일치 확인. 일치 없이 "완료" 보고 금지. (Stage 30.7 / fix / fix2 — push 누락 3연속. fix2 헤더 정정부터 자가확인 도입해 끊음.) **반복 금지.**

63. **출력 유도는 출력에 데이터가 있을 때만 유효 — 도구 간 응답 노출 비대칭이 투명성 결함** — 같은 데이터(merged preset/filter)를 보유해도 도구별 응답 노출이 다르면 투명성 결함이 생긴다. scan_preview는 filter_summary를 응답에 노출했으나 scan_execute는 누락 → 사용자가 어떤 업종 제외·어느 시장 검색인지 모른 채 결과 수령(피드백 2). 교훈 둘: (1) **"데이터 존재 ≠ 사용자 전달"** — 핸들러 스코프에 데이터가 있어도 응답에 안 실으면 client가 풀 수 없다. description 유도는 출력에 데이터가 실린 뒤에야 유효(fix2는 입력 유도/입력 스키마에 데이터 있음, 이번은 출력 유도/출력에 데이터 필요 — 둘은 동형 패턴이나 데이터 위치가 다름). (2) **유도 효과는 코드 검증 불가** — "client가 자연어로 푸는지"는 client 행동 의존, 단위 테스트는 "데이터가 응답에 있다"까지만 보증. field-test로만 확인(C-2). **baseline**: Stage 30.8 — execute 출력 격차를 0단계 회수로 확정(잠정 description-only 기각) → preset_used·filter_summary additive 추가. 호출부 additive 무해는 "추론 아닌 회수 확정"으로 처리(buildResponse 호출부 5곳 전부 직결 return, 형태 의존 0 — grep 확정. #60/#61의 "추론을 확정으로 다룸" 반복 차단).

64. **required 입력도 client 거짓 주입을 못 막는다 — 강제는 제어 흐름뿐, 도구 형태 무관** — 도구를 멈추게 하려고 신호를 *필수 입력(required)*으로 박아도 강제되지 않는다. client는 required든 optional이든 임의값을 주입한다(field-test: `min_opportunity_score=50` 사용자 요청 없이 주입). 강제는 오직 **제어 흐름**(신호 부재 시 실행 거부·견적 반환)으로만 성립하며, 도구 분리/단일 형태와 무관하다(강제력 동률). client가 신호를 거짓 주입하면 7부 위반 책임이 client로 이전 — 도구가 보장하는 범위는 "기본 상태에선 안 멈추고 못 간다"까지. #63(유도만으론 강제 불가)의 후속 — #63은 "출력/입력 *유도*(description)는 강제 못 함", #64는 "입력 *required*도 강제 못 함, 제어 흐름이라야". **baseline**: Stage 31 — 직전 진단의 자기정정("빡센 분리=필수 입력이면 강제 가능")을 ADR-0030 회수(현 게이트 + min_opportunity_score 무단 주입)로 재반증. "required로 막자"가 다음 세션에 또 나오지 않도록 박음.

65. **게이트 임계를 "한도 %"로 잡으면 절대 규모 폭주를 놓친다 — 막을 대상을 행위(한 스캔 규모)로 정의하라** — ADR-0030 게이트를 `usagePct > 100`(= 추정콜/DAILY_LIMIT × 100 > 100)으로 박았으나, field-test 폭주(전체 universe 빈 호출 → DART 16,002콜)는 하루 한도(20,000) *내* 80%라 게이트를 통과해 완주·중단했다. 막아야 할 대상은 "한도 초과"가 아니라 **"한 스캔 한 방의 절대 규모가 큼"**이었는데 임계를 비율·초과로 정의해 영영 안 잡혔다. 추가로 분모가 하루 *총* 한도라 기 사용분(잔여 한도)을 무시 — 같은 80% 스캔이 잔여 4,000 상황에서도 미발동. 교훈: (1) 멈춤 임계는 **보호하려는 행위 자체의 규모**(추정 콜수/universe 크기 절대값)로 정의 — 자원 잔량 비율이 아니라. (2) 설계 시 "막으려는 게 *초과*인가 *규모*인가"를 분리. field-test 폭주는 늘 *규모*였지 초과가 아니었다. (3) 실행 *중* checkpoint(ADR-0014)는 사후 복구지 ADR-0030 대화 루트(사전 견적)가 아님 — 둘이 혼동되면 "막았다"는 착각. **baseline**: Stage 32 후 field-test(`e206a41`) — scope_confirmed 분기·usagePct 계산식 회수로 빌드 미갱신 배제, 설계 정의 오류 확정. ADR-0030 재개정(임계 = 절대 규모) 다음 사이클.

66. **게이트 임계 = 절대 규모, 한도 % 아님 / 빈 호출 ≠ 빈 필터** — (1) scan_execute 빈 호출 `{}`은 빈 필터가 아니다. `resolveInput`(:252~266)이 default preset + user-preference(induty_whitelist/blacklist)를 resolved에 채워 필터가 작동한다. 실측: 빈 호출 universe=3,967 → preset 필터 → 2,221. (2) estimate 입력 universe = matched_cached + cache_miss (scan-execute.ts:822). 응답의 after_static_filter와 다를 수 있으나 빈 호출에선 같음(2,221). (3) 이 사실 누락이 이번 사이클 손계산 3회 오류(universe를 3,967 또는 2,221로 혼동)의 공통 뿌리. 실측 [GATE-DIAG] 파일 로그가 정리. (4) 게이트 임계는 한도 %(usagePct)가 아니라 스캔 한 방의 절대 콜수(`estimate.total > N`). 한도 내 큰 스캔(89.4%, 17,875콜)을 한도 % 임계로는 못 잡음. ADR-0030 재개정, N=10000(`SCAN_SCALE_GATE_CALLS`). (5) 검증 교훈: 빌드 미반영 시 console.error는 Claude Code가 stderr 캡처 안 해 유실. `fs.appendFileSync` 파일 로그 + 빌드 grep 검증(`grep -c build/...`)이 확실. **baseline**: ADR-0030 재개정(2026-06-02), field-test 실측 estimate.total=17875, 학습 #65 후속 확증.
