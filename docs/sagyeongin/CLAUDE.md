...
Stage 31 종결 (2026-05-29). ADR-0028 결정 — pre-check 2-phase cache 기반 induty 사전 필터. 결정 사이클 (code 0). 단테 1322 유지.

인계: Stage 32 (ADR-0028 구현) — isMarketMatch/isIndustryMatch `_lib` 추출 + Phase 2 분할 helper(`getCorpMeta` 기반 pure) + pre-check(scan-execute 729~739)/scan-preview 추정부 결선 교체 + estimated_universe_after_cache_filter + cache_coverage additive + spec §10.7/§10.8 개정 + cache warm/cold/부분 테스트. 이후 Stage 30.6 (default preset redesign). override(allow_over_daily_limit)는 ADR-0019 후속 별 사이클.
