...
Stage 30.4 scan_preview filter_summary KSIC list 명시 사이클 종결 (2026-05-28). `buildFilterSummary` 함수 추출 + `excluded_industries` (KSIC list) 응답 노출 정착. TOOL_REGISTRY 30 (사경인 15). Stage 30.4.1 (한도 초과 메시지 노출) 별 사이클 baseline 인계.
...
**통합 단테**: 7 files / +74 / -8 = **+66 net** (`src/lib/` 변경 0 — β-i 가드 정합).
...
**통합 단테**: 3 files / +83 / -6 = **+77 net** (`src/lib/` 변경 0 — β-i 가드 정합).
...
- cleanup 백로그: industry-merge.test.ts line 4 주석 "영역 본질" noise 정정 + 30.2~30.4 단테 누적 line 소급.
