...
3. D1 본질 영역 — fail-fast 카운터 추가 vs 본 흐름 만족 (옵션 α/β/γ → 본질 정정 후 D-i)
...
8. D1 본질 정정 후 input args 영역 (random_seed만 — D1 임계 영역 X)
...
| 3 | **옵션 D-i 확정 (β → D-i 정정)** — A2 강화 후 모든 catch (stage1/2/3 + enrich)가 1번 throw로 즉시 종결 → 본질적 fail-fast. 별도 카운터 추가 X (코드 변경 0). 묶음 3 명세 작성 사전 view 단계의 본질 분석으로 추가 정정 — 14(b) ②-plus 패턴 누적 |
...
| 8 | input args `random_seed?: number`만 추가 (D-i 정정으로 `consecutive_fetch_failed_threshold` 영역 X), state.input_args 보존 |
...
### 최종 결정 — **옵션 D-i 정정 채택 (β → D-i)**

본 사전 검증 영역 본문 작성 후 묶음 3 명세 작성 사전 view 진입 시점에 추가 본질 분석:

- A2 강화 후 1차 fetch failed → sleep + retry → 2차 fetch failed → `DartRateLimitError` throw
- scan-execute의 모든 catch (stage1StaticFilter line 252-260 + Stage 2 line 700 + Stage 3 line 754 + enrich line 365/387/406/422) → *첫 throw로 즉시 종결*
- → 본 흐름이 *retry-and-throw 1번 즉시 종결 = fail-fast 본질*. 별도 카운터 추가는 *발동 시점 전에 의미 X*

ADR-0015 line 92의 *연속 N회 임계*는 *retry 흡수 영역* (1차 + retry = N=2)으로 재해석 — A2 정책의 retry 1회 = 본 임계 만족.

**옵션 D-i 채택**: 코드 변경 0. ADR-0015 D1 본문 갱신 (별도 docs commit). 묶음 3 변경 면 절감 (D1 영역 제거, input args `random_seed`만).

근거:
- A2 강화로 retry 후 throw가 영구 차단 신호 propagate
- scan-execute 모든 catch가 첫 throw로 즉시 종결 (β의 *5부 사상 보존* 자동 만족)
- 추가 카운터 = 본질 영역 X + 변경 면 추가
- 옵션 β의 stage1/2/3 카운터도 *1번 catch로 즉시 종결*이라 의미 X — D-i가 본질 정합

본 정정은 14(b) 정착 패턴 ②-plus (사전 검증 작업이 정책 후보 자체 재정의) 누적 사례 — 본 사전 검증 단계에서 α→β 정정 후 묶음 3 사전 view 단계에서 β→D-i 추가 정정.
...
| **3** | B1 + C1 호출자 적용 (D-i 정정 후 D1 제거) | `scan-helpers.ts`: `shuffleWithSeed` + `mulberry32` 추가. `scan-execute.ts`: universe shuffle + input args `random_seed` (D1 영역 X). srim.ts + required-return.ts의 naver/KIS 호출자 wrapper 경유 변경 | `scan-helpers.test.ts` 14 → 17 (+3). D1 단테 영역 X |
...
## 영역 8: D-i 정정 후 input args 영역 (random_seed만)
...
D-i 정정으로 `consecutive_fetch_failed_threshold` 영역 X (D1 본질이 흐름 만족).
